import streamlit as st
import yfinance as yf
import pandas as pd
import plotly.graph_objects as go
import numpy as np
from datetime import datetime

# 1. 페이지 설정
st.set_page_config(page_title="주간 상승률 분석기", layout="wide")

# 2. 종목 및 기본 설정
tickers_info = {
    '005930.KS': {'name': '삼성전자', 'color': '#0057D8', 'width': 5},
    'CL=F': {'name': 'WTI 원유', 'color': '#E67E22', 'width': 1.5},
    'DX-Y.NYB': {'name': '달러지수(x5)', 'color': '#34495E', 'width': 1.5},
    'SI=F': {'name': '글로벌 은', 'color': '#BDC3C7', 'width': 1.5}
}

@st.cache_data(ttl=60) # 1분 간격 캐시
def get_weekly_performance_data():
    combined_df = []
    current_stats = {}
    
    for sym, info in tickers_info.items():
        try:
            # 1개월치 15분봉 데이터 로드
            df = yf.download(sym, period='1mo', interval='15m', progress=False)
            if df.empty: continue
            
            # MultiIndex 처리: yf.download 결과 구조 대응
            if isinstance(df.columns, pd.MultiIndex):
                close = df['Close'][sym]
            else:
                close = df['Close']
            
            close = close.dropna()
            
            # 시간대 KST 변환
            if close.index.tz is None:
                close.index = close.index.tz_localize('UTC').tz_convert('Asia/Seoul')
            else:
                close.index = close.index.tz_convert('Asia/Seoul')

            # --- [핵심] 월요일 아침 첫 가격 기준 수익률 계산 ---
            # 연도-주차(ISO 기준)로 그룹화
            year_week = close.index.strftime('%G-%V') 
            
            def get_first_valid(series):
                return series.iloc[0] if len(series) > 0 else np.nan

            # 각 주차별 첫 가격(월요일 시가)을 구해서 해당 주차의 모든 행에 매핑
            monday_open_price = close.groupby(year_week).transform(get_first_valid)
            
            # 월요일 아침 대비 상승률 (%)
            ret = ((close - monday_open_price) / monday_open_price * 100)
            
            # 달러지수 가중치 (x5) 적용
            if sym == 'DX-Y.NYB': 
                ret = ret * 5
            
            # 실시간 수치 저장
            latest_val = close.iloc[-1]
            latest_ret = ret.iloc[-1]
            current_stats[sym] = {'price': latest_val, 'ret': latest_ret}

            ret.name = sym
            combined_df.append(ret)
        except Exception as e:
            print(f"Error loading {sym}: {e}")
            continue
        
    return pd.concat(combined_df, axis=1) if combined_df else (None, {})

def run_app():
    st.title("📊 주간 상승률 비교 대시보드")
    st.markdown("##### 🟦 삼성전자 강조 | ⬛ 월요일 시가(0%) 기준 변동 | ✂️ 장외 시간 제외")

    df, stats = get_weekly_performance_data()
    
    if df is None or not stats:
        st.error("데이터를 불러오지 못했습니다. 잠시 후 다시 시도해주세요.")
        return

    fig = go.Figure()

    # 가독성을 위해 삼성전자를 마지막에 그려서 위로 올림
    plot_order = ['CL=F', 'DX-Y.NYB', 'SI=F', '005930.KS']
    
    for sym in plot_order:
        if sym in df.columns:
            info = tickers_info[sym]
            curr = stats.get(sym, {'price': 0, 'ret': 0})
            
            # 범례에 현재가와 수익률 표시
            display_name = f"{info['name']} ({curr['ret']:+.2f}%)"
            
            fig.add_trace(go.Scatter(
                x=df.index, y=df[sym],
                name=display_name,
                line=dict(color=info['color'], width=info['width']),
                connectgaps=True,
                hovertemplate=f"<b>{info['name']}</b>: %{{y:.2f}}%<extra></extra>"
            ))

    # 월요일 개장 시점(09:00) 세로선 표시
    monday_starts = df.index[(df.index.weekday == 0) & (df.index.hour == 9) & (df.index.minute == 0)]
    for m_start in monday_starts:
        fig.add_vline(x=m_start, line_width=1, line_dash="dash", line_color="gray")

    fig.update_layout(
        hovermode="x unified",
        height=700,
        margin=dict(l=20, r=20, t=50, b=20),
        template="plotly_white",
        xaxis=dict(
            title="일시 (KST)",
            tickformat="%m/%d\n%H:%M",
            rangebreaks=[
                dict(bounds=["sat", "mon"]),           # 토~월요일 개장 전 삭제
                dict(bounds=[15.5, 9], pattern="hour")  # 15:30 ~ 09:00 삭제
            ]
        ),
        yaxis=dict(
            title="상승률 (%)",
            range=[-15, 15], # 현실적인 등락 폭으로 조정
            ticksuffix="%",
            zeroline=True, 
            zerolinewidth=2, 
            zerolinecolor='black'
        ),
        legend=dict(
            orientation="h", 
            yanchor="bottom", 
            y=1.02, 
            xanchor="right", 
            x=1
        )
    )

    st.plotly_chart(fig, use_container_width=True)
    
    # 하단 요약 정보 카드
    cols = st.columns(len(tickers_info))
    for i, (sym, info) in enumerate(tickers_info.items()):
        if sym in stats:
            cols[i].metric(info['name'], f"{stats[sym]['price']:,.0f}", f"{stats[sym]['ret']:.2f}%")

if __name__ == "__main__":
    run_app()
