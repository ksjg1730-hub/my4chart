import streamlit as st
import yfinance as yf
import pandas as pd
import plotly.graph_objects as go
import numpy as np
from datetime import datetime

# 1. 페이지 설정
st.set_page_config(page_title="주간 상승률 분석기", layout="wide")

# 2. 종목 및 기본 설정
tickers_info = {
    '005930.KS': {'name': '삼성전자', 'color': '#0057D8', 'width': 5},
    'CL=F': {'name': 'WTI 원유', 'color': '#E67E22', 'width': 2},
    'DX-Y.NYB': {'name': '달러지수(x5)', 'color': '#34495E', 'width': 2},
    'SI=F': {'name': '글로벌 은', 'color': '#BDC3C7', 'width': 2}
}

@st.cache_data(ttl=60)
def get_weekly_performance_data():
    combined_df = []
    current_stats = {}

    for sym, info in tickers_info.items():
        try:
            # 1개월치 15분봉 데이터 로드 (group_by='ticker'로 인덱스 고정)
            df = yf.download(sym, period='1mo', interval='15m', progress=False, group_by='ticker')
            if df.empty: continue
            
            # 멀티인덱스 또는 단일인덱스 대응
            if isinstance(df.columns, pd.MultiIndex):
                close = df['Close'].iloc[:, 0].copy()
            else:
                close = df['Close'].copy()
            
            # 시간대 KST 변환
            if close.index.tz is None:
                close.index = close.index.tz_localize('UTC').tz_convert('Asia/Seoul')
            else:
                close.index = close.index.tz_convert('Asia/Seoul')

            # --- 월요일 아침 첫 가격 기준 수익률 계산 ---
            # 연도-주차 계산
            year_week = close.index.strftime('%Y-%U')
            
            # 해당 주의 첫 번째 가격을 찾아서 매칭
            monday_opens = close.groupby(year_week).transform('first')
            
            # 수익률 계산 (%)
            ret = ((close - monday_opens) / monday_opens * 100)
            
            # 달러지수 가중치 (x5)
            if sym == 'DX-Y.NYB': ret *= 5
            
            # 실시간 수치 저장용
            latest_val = close.dropna().iloc[-1]
            latest_ret = ret.dropna().iloc[-1]
            current_stats[sym] = {'price': latest_val, 'ret': latest_ret}

            ret.name = sym
            combined_df.append(ret)
        except Exception as e:
            print(f"Error loading {sym}: {e}")
            continue
    
    return pd.concat(combined_df, axis=1) if combined_df else (None, None), current_stats

def run_app():
    st.title("📊 월요일 아침 대비 주간 수익률 추이")
    st.markdown("##### 🟦 삼성전자 강조 | ⬛ 월요일 0% 시작 | ✂️ 휴장 시간 제외")

    df, stats = get_weekly_performance_data()
    
    if df is None or df.empty:
        st.error("데이터를 가져오지 못했습니다. 잠시 후 다시 시도해주세요.")
        return

    fig = go.Figure()

    # 종목 그리기 (삼성전자를 가장 마지막에 추가하여 레이어 최상단 배치)
    plot_order = ['CL=F', 'DX-Y.NYB', 'SI=F', '005930.KS']

    for sym in plot_order:
        if sym in df.columns:
            info = tickers_info[sym]
            curr = stats.get(sym, {'price': 0, 'ret': 0})
            
            # 이름 옆에 현재가 및 수익률 표시
            display_name = f"{info['name']} [{curr['price']:,.0f} | {curr['ret']:+.2f}%]"
            
            fig.add_trace(go.Scatter(
                x=df.index, y=df[sym],
                name=display_name,
                line=dict(color=info['color'], width=info['width']),
                connectgaps=True,
                hovertemplate=f"<b>{info['name']}</b><br>수익률: %{{y:.2f}}%<extra></extra>"
            ))

    # 기준선 (매주 월요일 장 시작 시점 수직선)
    monday_starts = df.index[(df.index.weekday == 0) & (df.index.hour == 9) & (df.index.minute == 0)]
    for m_start in monday_starts:
        fig.add_vline(x=m_start, line_width=1.5, line_dash="dash", line_color="black")

    fig.update_layout(
        hovermode="x unified",
        height=700,
        template="plotly_white",
        xaxis=dict(
            tickformat="%m/%d %H:%M",
            rangebreaks=[
                dict(bounds=["sat", "mon"]),           # 주말 제거
                dict(bounds=[15.5, 9], pattern="hour"), # 한국 장외 시간 제거 (15:30~09:00)
            ]
        ),
        yaxis=dict(
            title="수익률 (%)",
            range=[-15, 15],   # 시야 확보를 위해 ±15%로 조정
            ticksuffix="%",
            zeroline=True, zerolinewidth=2, zerolinecolor='black'
        ),
        legend=dict(orientation="h", y=1.05, x=0.5, xanchor="center", font=dict(size=12))
    )

    st.plotly_chart(fig, use_container_width=True)
    st.info(f"💡 마지막 데이터 업데이트: {df.index[-1].strftime('%Y-%m-%d %H:%M:%S')}")

if __name__ == "__main__":
    run_app()
